...
examples:
  ...
  - /components/note.wireframe.html
---
...
# Structure
A flex row aligned to the top, not centred, inside `12px 14px` of padding.

| Part | Order | Sizing | Space after | Reflow |
|------|-------|--------|-------------|--------|
| `.icon` | 1 | fixed (`flex: none`) | `10px` | unchanged |
| Message | 2 | fills the remaining width, wraps | none | unchanged |

Default alignment rather than `align-items: center` is what keeps the icon beside the first line
when the message wraps to several lines, which is the common case for a note. `flex: none` stops
the icon being squeezed by a long message. All four severities share this box exactly and differ
only in border and fill.
