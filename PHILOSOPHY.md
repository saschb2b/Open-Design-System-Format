...
— plain markdown files, one-field conformance, cross-links, progressive disclosure, no SDK, no
...
copied. Prose sets the intent; the example sets the contract.
...
It is tempting to make a format strict — to demand every token, every state, every field, and
reject anything incomplete. That instinct kills adoption. A half-documented design system that an
agent can still partly use beats a perfect one nobody finished writing.
